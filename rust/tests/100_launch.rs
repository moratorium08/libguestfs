...
fn launch() {
    ...
    g.add_drive_scratch(500 * 1024 * 1024, Default::default()).expect("add_drive_scratch");
    ...
    assert_eq!(lvs, vec!["/dev/VG/LV1".to_string(), "/dev/VG/LV2".to_string()]);

    g.mkfs("ext2", "/dev/VG/LV1", Default::default()).expect("mkfs");
